...
module Unifapi
  ...
  class BrowserScreenshotRequestViewport
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unifapi::BrowserScreenshotRequestViewport` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unifapi::BrowserScreenshotRequestViewport`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @height > 3840
        invalid_properties.push('invalid value for "height", must be smaller than or equal to 3840.')
      end
    ...
    def valid?
      ...
      return false if @width > 3840
      return false if @width < 1
      return false if @height.nil?
      return false if @height > 3840
    ...
    def height=(height)
      ...
      if height > 3840
        fail ArgumentError, 'invalid value for "height", must be smaller than or equal to 3840.'
      end
