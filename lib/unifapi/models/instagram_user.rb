  ...
  class InstagramUser
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unifapi::InstagramUser` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unifapi::InstagramUser`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'media_count')
        ...
        self.media_count = nil
      end

      if attributes.key?(:'fbid_v2')
        self.fbid_v2 = attributes[:'fbid_v2']
      else
        self.fbid_v2 = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @following_count.nil?
        invalid_properties.push('invalid value for "following_count", following_count cannot be nil.')
      end

      if @media_count.nil?
        invalid_properties.push('invalid value for "media_count", media_count cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @following_count.nil?
      return false if @media_count.nil?
      true
    end
    ...
    def hash
      [id, user_id, username, full_name, is_verified, is_private, profile_pic_url, follower_count, following_count, media_count, fbid_v2].hash
    end
