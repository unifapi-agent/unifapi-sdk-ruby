  ...
  class YouTubeVideoPreview
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unifapi::YouTubeVideoPreview` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unifapi::YouTubeVideoPreview`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @published_time.nil?
        invalid_properties.push('invalid value for "published_time", published_time cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @published_time.nil?
    ...
    def hash
      [id, title, author, channel_id, description, duration_seconds, view_count, published_time, is_live, category].hash
    end
