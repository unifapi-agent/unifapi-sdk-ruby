...
module Unifapi
  class XUser
    ...
    attr_accessor :profile_banner_url
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unifapi::XUser` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unifapi::XUser`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'profile_banner_url')
        self.profile_banner_url = attributes[:'profile_banner_url']
      end
    ...
    def hash
      [id, name, username, created_at, description, location, url, profile_image_url, profile_banner_url, protected, verified, verified_type, pinned_tweet_id, public_metrics].hash
    end
